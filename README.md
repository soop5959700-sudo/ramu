...
- 🎵 노래책 · ⚡ 업보 · 📔 일기 · 👗 옷장 · ✉️ 문의함
- 🖼 메인 커버 사진: 배경(16:9) + 폴라로이드 2장(3:4) — 비우면 기본 사진
- ✏️ 문구: 6개 페이지의 머리글(라벨·제목·영문·설명)
- 🎨 테마: 색 **7종**(메인 4단계 + 서브 + 배경 + 로고) + 글자 크기 4단 배율

  색을 바꾸면 버튼뿐 아니라 **페이지 배경·카드·분홍 포인트까지** 같이 바뀝니다.
  사이트의 모든 색이 이 7개에서 파생되도록 묶여 있어요.
