...
var FX_FLOAT = ['✦'];              // unused here: ambient float is the bubble system below
var FX_CLICK = '🌰';               // click / avatar-tap particle
var FX_COUNT = 0;                  // template float disabled; bubbles handle ambience
var FX_TILT  = true;

var FX_LOADER      = true;
var FX_LOADER_IMG  = '';     // empty = auto: --char mascot, then SOOP favicon, then text
var FX_LOADER_TEXT = '라무';
var FX_TRANS_MS    = 800;
...
(function () {
  ...
  function buildLoader() {
    ...
    var logoTxt = ((document.querySelector('.nav-brand, .nav-logo') || {}).textContent || document.title || '').trim();
  ...
  function build() {
    ...
    if (FX_TILT && mqFine && !mqReduce && !window.__fxTiltOn) {
      ...
      var TILT_SEL = '.card, .glass-panel, .item-card, .viewer-card, .poster, .up-item, .ovod-card';
      var TILT_DEG = 2.5;                                   
      var _tiltEl = null;
...
/* Ambient bubble drift from the confirmed main handoff. Uses #fx + .fxfb (style.css). */
(() => {
  if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
  const root = document.querySelector("#fx") || document.body.appendChild(Object.assign(document.createElement("div"), { id: "fx" }));
  ...
  const makeBubble = () => {
    ...
    bubble.className = "fxfb";
    bubble.style.left = `${Math.random() * 100}vw`;
    bubble.style.top = "-18px";
    bubble.style.setProperty("--size", `${size}px`);
    bubble.style.setProperty("--opacity", `${0.18 + Math.random() * 0.28}`);
    bubble.style.setProperty("--drift", `${-60 + Math.random() * 120}px`);
    bubble.style.setProperty("--duration", `${7 + Math.random() * 7}s`);
    root.appendChild(bubble);
    bubble.addEventListener("animationend", () => bubble.remove(), { once: true });
  };

  window.fxBubbles = window.fxBubbles || (() => {
    let count = 0;
    const timer = window.setInterval(() => {
      if (document.hidden) return;
      makeBubble();
      count += 1;
      if (count >= limit) window.clearInterval(timer);
    }, compact ? 920 : 720);
  });

  const start = () => {
    if (typeof window.fxBubbles === "function") window.fxBubbles();
  };
